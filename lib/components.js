...
/** Context has the following props:
 *  - db
 *  - setFlow()
 *  - it: reference to last cursor
 */
let context = null;
let state = {
  id: 'INIT',
...
vue.component('db', {
  ...
  template: `
    <div class="db">
      ...
      <div class="results">
        ...
        <div v-if="!results">
          No Results to Show. Try running a command!
        </div>
      </div>
    </div>
  `,
  methods: {
    run: function(command) {
      actions.execCommand(command);
    },
    queryCollection: function(collection) {
      this.command = `db.${collection}.find()`;
      actions.execCommand(this.command);
    }
  }
});
...
function initialize() {
  new vue({
    ...
    template: `
      ...
      <db v-if="state.id === 'CONNECTED'" :results="state.results"></db>
    `,
    data: {
      state: state
    }
